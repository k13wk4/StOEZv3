document.addEventListener("DOMContentLoaded", () => {
    ...
    function findMatches(query, workshop, line) {
        ...
        let filteredByDropdowns = allData;
        if (workshop) filteredByDropdowns = filteredByDropdowns.filter(item => item["Цех"] === workshop);
        if (line) filteredByDropdowns = filteredByDropdowns.filter(item => String(item["Лінія"]) === line);

        if (!cleansedQuery) {
            return filteredByDropdowns.map(item => ({ item, contextMatches: [] }));
        }

        const resultsWithContext = [];
        for (const item of filteredByDropdowns) {
            const contextMatches = [];
            let topLevelMatch = cleanseString(item["Код обладнання"]).includes(cleansedQuery) || cleanseString(item["Назва обладнання (українською)"]).includes(cleansedQuery);

            item["ЕМ/КВПіА"]?.forEach(em => {
                if (Object.values(em).some(val => typeof val === 'string' && cleanseString(val).includes(cleansedQuery))) {
                    contextMatches.push({ ...em, type: 'ЕМ' });
                }
                em["КВПіА"]?.forEach(kvp => {
                    if (Object.values(kvp).some(val => typeof val === 'string' && cleanseString(val).includes(cleansedQuery))) {
                        contextMatches.push({ ...kvp, type: 'КВПіА' });
                    }
                });
            });

            if (topLevelMatch || contextMatches.length > 0) {
                resultsWithContext.push({ item, contextMatches, topLevelMatch });
            }
        }
    ...
    function displayResults(resultsWithContext, query) {
        ...
        resultsWithContext.forEach(({ item, contextMatches, topLevelMatch }) => {
            ...
            if (!topLevelMatch && contextMatches.length > 0) {
                subItemsHtml = contextMatches.map(subItem => {
                    if (subItem.type === 'ЕМ') {
                        return `<div class="mt-2 p-2 bg-red-50 border border-red-200 rounded-lg text-sm"><b class="text-red-700">Збіг в ЕМ:</b> ${highlightMatch(subItem["ЕМ"], query)} | <b>Шафа:</b> ${highlightMatch(subItem["Номер шафи EM"] || '-', query)} | <b>Автомат:</b> ${highlightMatch(subItem["Номер Автомата"] || '-', query)}</div>`;
                    }
                    if (subItem.type === 'КВПіА') {
                        return `<div class="mt-2 p-2 bg-blue-50 border border-blue-200 rounded-lg text-sm"><b class="text-blue-700">Збіг в КВПіА:</b> ${highlightMatch(subItem["Назва датчика"], query)} | <b>Шафа:</b> ${highlightMatch(subItem["Номер шафи КВПіА"] || '-', query)} | <b>Сигнал:</b> ${highlightMatch(subItem["Номер сигналу"] || '-', query)}</div>`;
                    }
                    return '';
                }).join('');
            } else {
    ...
    function populateFilters() {
        ...
        workshops.forEach(w => workshopFilter.innerHTML += `<option value="${w}">Цех ${w}</option>`);
        lines.forEach(l => lineFilter.innerHTML += `<option value="${l}">Лінія ${l}</option>`);
    }
