document.addEventListener("DOMContentLoaded", () => {
    ...
    function displayResults(results, query) {
        ...
        results.forEach(item => {
            ...
            if (!topLevelMatch && query) {
                ...
                item["ЕМ/КВПіА"]?.forEach(em => {
                    if (Object.values(em).some(val => typeof val === 'string' && cleanseString(val).includes(cleanseString(query)))) contextMatches.push({ ...em, type: 'ЕМ' });
                    em["КВПіА"]?.forEach(kvp => {
                        if (Object.values(kvp).some(val => typeof val === 'string' && cleanseString(val).includes(cleanseString(query)))) contextMatches.push({ ...kvp, type: 'КВПіА' });
                    });
                });
                subItemsHtml = contextMatches.map(subItem => {
                    if (subItem.type === 'ЕМ') return `<div class="mt-2 p-2 bg-red-50 border border-red-200 rounded-lg text-sm"><b class="text-red-700">Збіг в ЕМ:</b> ${highlightMatch(subItem["ЕМ"], query)} | <b>Шафа:</b> ${highlightMatch(subItem["Номер шафи EM"] || '-', query)} | <b>Автомат:</b> ${highlightMatch(subItem["Номер Автомата"] || '-', query)}</div>`;
                    if (subItem.type === 'КВПіА') return `<div class="mt-2 p-2 bg-blue-50 border border-blue-200 rounded-lg text-sm"><b class="text-blue-700">Збіг в КВПіА:</b> ${highlightMatch(subItem["Назва датчика"], query)} | <b>Шафа:</b> ${highlightMatch(subItem["Номер шафи КВПіА"] || '-', query)} | <b>Сигнал:</b> ${highlightMatch(subItem["Номер сигналу"] || '-', query)}</div>`;
                    return '';
                }).join('');
            } else {
    ...
    function highlightMatch(text, query) {
        ...
        const cleansedQuery = cleanseString(query);
        if (!cleansedQuery) return text;
        const regex = new RegExp(cleansedQuery.split('').join('[\\s\\.,_-]*'), 'ig');
    ...
    function applyUrlParamsOnLoad() {
        ...
        const tseh = params.get('tseh') || '';
        const line = params.get('line') || '';
        searchInput.value = query;
        workshopFilter.value = tseh;
    ...
    function updateFiltersState(query, selectedWorkshop, selectedLine) {
        const relevantForWorkshops = findMatches(query, '', selectedLine);
        const workshopCounts = {};
        for (const item of relevantForWorkshops) {
            workshopCounts[item["Цех"]] = (workshopCounts[item["Цех"]] || 0) + 1;
        }

        workshopFilter.querySelectorAll('option').forEach(option => {
            const baseText = option.dataset.baseText;
            if (!baseText) return;
            const count = workshopCounts[option.value] || 0;
            option.textContent = count > 0 ? `${baseText} (${count})` : baseText;
            option.disabled = count === 0;
        });

        const relevantForLines = findMatches(query, selectedWorkshop, '');
        const lineCounts = {};
        for (const item of relevantForLines) {
            lineCounts[item["Лінія"]] = (lineCounts[item["Лінія"]] || 0) + 1;
        }

        lineFilter.querySelectorAll('option').forEach(option => {
            const baseText = option.dataset.baseText;
            if (!baseText) return;
            const count = lineCounts[option.value] || 0;
            option.textContent = count > 0 ? `${baseText} (${count})` : baseText;
            option.disabled = count === 0;
        });
    }
    ...
    function setupEventListeners() {
        ...
        closeModalButton.addEventListener('click', () => {
            ...
            schematicModal.classList.remove('flex');
            schematicImage.src = '';
        });
    ...
    function addToggleDetailsListeners() {
        document.querySelectorAll(".toggle-details").forEach(button => button.addEventListener("click", () => {
            const target = document.getElementById(button.dataset.target);
            const mainIcon = button.querySelector('.toggle-icon');
    ...
    function addSchematicButtonListeners() {
        document.querySelectorAll(".open-schematic-btn").forEach(button => button.addEventListener("click", event => {
            event.stopPropagation();
            const schematicFilename = button.dataset.schematic;
            if (schematicFilename) {
                schematicImage.src = `./${schematicFilename}`;
                schematicModal.classList.remove('hidden');
                schematicModal.classList.add('flex');
            }
        }));
    }
